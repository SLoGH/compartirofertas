...
add_action( 'admin_print_styles', 'th_contactform_admin_css' );
function th_contactform_admin_css() {
...
#icon-edit, #icon-post { background: url("<?php echo CONTACT_FORM_SHORTCODE_URL; ?>/images/menu-big.png") no-repeat !important; }
</style>
...
add_action( 'admin_menu', 'th_contactform_admin_menu' );
function th_contactform_admin_menu() {
	global $menu, $submenu;
	unset( $submenu['edit.php?post_type=feedback'] );
}

add_filter( 'bulk_actions-edit-feedback', 'th_contactform_admin_bulk_actions' );
function th_contactform_admin_bulk_actions( $actions ) {
	global $current_screen;
	if ( 'edit-feedback' != $current_screen->id )
		return $actions;
...
add_filter( 'views_edit-feedback', 'th_contactform_admin_view_tabs' );
function th_contactform_admin_view_tabs( $views ) {
	global $current_screen;
	if ( 'edit-feedback' != $current_screen->id )
		return $actions;
...
add_filter( 'manage_feedback_posts_columns', 'th_contactform_post_type_columns_filter' );
function th_contactform_post_type_columns_filter( $cols ) {
	$cols = array(
		'cb'	=> '<input type="checkbox" />',
		'feedback_from'		=> __( 'From', 'churchope' ),
		'feedback_message'		=> __( 'Message', 'churchope' ),
		'feedback_date'			=> __( 'Date', 'churchope' )
	);

	return $cols;
}

add_action( 'manage_posts_custom_column', 'th_contactform_manage_post_columns', 10, 2 );
function th_contactform_manage_post_columns( $col, $post_id ) {
	global $post;
	...
	switch ( $col ) {
		...
		case 'feedback_message':
			...
			if ( $post->post_status == 'trash' ) {
				...
				echo '"><a title="';
				echo esc_attr__( 'Resore this item from the Trash', 'churchope' );
				...
				echo '">' . __( 'Restore', 'churchope' ) . '</a></span> | ';

				echo "<span class='delete'> <a class='submitdelete' title='";
				echo esc_attr( __( 'Delete this item permanently', 'churchope' ) );
				echo "' href='" . get_delete_post_link( $post->ID, '', true );
				echo "'>" . __( 'Delete Permanently', 'churchope' ) . "</a></span>";
?>
...
$('#feedback-restore-<?php echo $post_id; ?>').click(function(e) {
	e.preventDefault();
	$.post(ajaxurl, {
			action: 'th_contactform_ajax_spam',
			post_id: '<?php echo $post_id; ?>',
			make_it: 'publish',
			sub_menu: jQuery('.subsubsub .current').attr('href'),
			_ajax_nonce: '<?php echo wp_create_nonce( 'th-contactform-post-status-' . $post_id ); ?>'
		},
		function(r) {
...
<?php
				...
				echo '"><a title="';
				echo __( 'Mark this message as spam', 'churchope' );
				...
				echo '">';
				echo '<a class="submitdelete" title="' . esc_attr__( 'Trash' , 'churchope');
				echo '" href="' . get_delete_post_link( $post_id );
				echo '">' . __( 'Trash' , 'churchope') . '</a></span>';
...
<?php
				...
				echo '"><a title="';
				echo __( 'Mark this message as NOT spam', 'churchope' );
...
function th_contactform_ajax_shortcode() {
	...
	if ( is_array( $_POST['fields'] ) ) {
		...
		foreach ( $_POST['fields'] as $field ) {
			...
			$shortcode .= '[contact-field label="'. $field['label'] .'" type="'.th_contactform_esc_attr($field['type']).'"' . $req . $opts .' /]'."\n";
		}
	}
...
add_action( 'wp_ajax_th_contactform_ajax_spam', 'th_contactform_ajax_spam' );
function th_contactform_ajax_spam() {
	global $wpdb;
	...
	check_ajax_referer( 'th-contactform-post-status-' . $post_id );
	if ( !current_user_can("edit_page", $post_id) )
		wp_die( __( 'You are not allowed to manage this item.', 'churchope' ) );
	...
	if ( $_POST['make_it'] == 'spam' ) {
		...
		if ( !current_user_can($post_type_object->cap->delete_post, $post_id) )
			wp_die( __('You are not allowed to move this item out of the Trash.', 'churchope') );

		if ( ! wp_untrash_post($post_id) )
			wp_die( __('Error in restoring from Trash.', 'churchope') );

	} elseif( $_POST['make_it'] == 'trash' ) {
		if ( !current_user_can($post_type_object->cap->delete_post, $post_id) )
			wp_die( __('You are not allowed to move this item to the Trash.', 'churchope') );

		if ( ! wp_trash_post($post_id) )
			wp_die( __('Error in moving to Trash.', 'churchope') );
	...
	if ( isset( $status['publish'] ) ) {
		...
		$status_html .= '>' . __( 'Messages', 'churchope' ) . ' <span class="count">';
	...
	if ( isset( $status['trash'] ) ) {
		...
		$status_html .= '>' . __( 'Trash', 'churchope' ) . ' <span class="count">';
	...
	if ( isset( $status['spam'] ) ) {
		...
		$status_html .= '>' . __( 'Spam', 'churchope' ) . ' <span class="count">';
